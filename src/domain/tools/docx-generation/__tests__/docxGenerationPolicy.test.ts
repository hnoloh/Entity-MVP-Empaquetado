...
describe('DocxGenerationPolicy', () => {
  ...
  beforeEach(() => {
    ...
    policy = new DocxGenerationPolicy(mockAuthRepo);
  });

  it('permite generar DOCX si esta autorizado y cumple requisitos', () => {
    vi.mocked(mockAuthRepo.isToolAuthorized).mockReturnValue(true);
    const result = policy.validate({ entiId: 'enti-1', toolId: 't1', content: 'hello', filename: 'doc.docx' });
    expect(result).toEqual({ allowed: true });
  });

  it('allows empty content per user override', () => {
    vi.mocked(mockAuthRepo.isToolAuthorized).mockReturnValue(true);
    expect(policy.validate({ entiId: 'enti-1', toolId: 't1', content: '', filename: 'doc.docx' })).toEqual({ allowed: true });
    expect(policy.validate({ entiId: 'enti-1', toolId: 't1', content: '   ', filename: 'doc.docx' })).toEqual({ allowed: true });
  });

  it('bloquea si owner es group', () => {
    const result = policy.validate({ entiId: 'group-1', toolId: 't1', content: 'hello', filename: 'doc.docx' });
  ...
  it('bloquea extension invalida o path traversal', () => {
    vi.mocked(mockAuthRepo.isToolAuthorized).mockReturnValue(true);
    expect(policy.validate({ entiId: 'enti-1', toolId: 't1', content: 'hello', filename: '../doc.docx' }).allowed).toBe(false);
