...
import { PdfGenerationToolExecutor } from '../pdfGenerationToolExecutor';
import { PdfGenerationPolicy } from '../pdfGenerationPolicy';
import { InMemoryGeneratedArtifactRegistry } from '../../generated-artifacts';

describe('PdfGenerationToolExecutor', () => {
  let executor: PdfGenerationToolExecutor;
  let policy: PdfGenerationPolicy;
  let registry: InMemoryGeneratedArtifactRegistry;
  let indicators: unknown;

  beforeEach(() => {
    policy = { validate: vi.fn() } as unknown as PdfGenerationPolicy;
    registry = new InMemoryGeneratedArtifactRegistry();
    indicators = { setIndicator: vi.fn() };
    executor = new PdfGenerationToolExecutor(policy, registry, indicators as any);
  });

  it('ejecuta con exito', async () => {
    vi.mocked(policy.validate).mockReturnValue({ allowed: true });
    const result = await executor.execute({ entiId: 'enti-1', toolId: 't1', content: 'hi', filename: 'a.pdf' });
  ...
  it('devuelve blocked si policy falla', async () => {
